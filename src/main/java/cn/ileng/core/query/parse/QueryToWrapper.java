package cn.ileng.core.query.parse;
...
import cn.ileng.core.query.data.Condition;
import cn.ileng.core.query.data.Condition.Filter;
import cn.ileng.core.query.data.Condition.Operator;
...
public class QueryToWrapper<T> {
	...
	public void parseSort(Wrapper<T> wrapper, Queryable queryable) {
		cn.ileng.core.query.data.Sort sort = queryable.getPageable().getSort();
		if (sort != null) {
			for (cn.ileng.core.query.data.Sort.Order order : sort) {
				if (order.getDirection() == cn.ileng.core.query.data.Sort.Direction.DESC) {
					wrapper.orderBy(order.getProperty(), false);
				} else if (order.getDirection() == cn.ileng.core.query.data.Sort.Direction.ASC) {
